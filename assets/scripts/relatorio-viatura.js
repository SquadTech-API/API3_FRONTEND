...
function downloadRelatorio(formato, idVeiculo) {

  // pega o filtro de período da tela
  const periodo = document.getElementById("filtropesquisa").value;

  // mapeia período para o backend
  const mapaPeriodo = {
    hoje: "hoje",
    semana: "7d",
    mes: "30d",
  ...
  const periodoFormatado = mapaPeriodo[periodo] || "30d";
  ...
  // 5. chama backend para download
  const url = `/relatorios/viatura?${params.toString()}`;
